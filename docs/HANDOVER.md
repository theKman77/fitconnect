...
> Demand Engine update (2026-07-20): one-tap repeat booking, private trainer waitlists, and 72-hour Pulse Drops are implemented. Waitlist identity/contact data is never exposed to trainers; trainers see only aggregate demand and private match counts. Real bookings remain the sole way to claim inventory, preserving server-authoritative pricing, availability, and marketplace protection. See migrations `20260720172618_demand_engine.sql` and `20260720174803_demand_engine_hardening.sql`.
