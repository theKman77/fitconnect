...
export default function SessionCelebration() {
  ...
  useEffect(() => {
    ...
    getBooking(id)
      ...
      .catch(() => setError(tr('Could not load this completed session.')));
  ...
  async function submit() {
    if (!isPreview && booking?.status !== 'completed') return setError(tr('Ratings unlock only after the trainer completes the session.'));
    setBusy(true);
    setError(null);
    try {
      if (!isPreview && isBackendConfigured && id && profile) {
        if (!trainer?.profile_id) throw new Error(tr('Trainer account is unavailable.'));
      ...
      setError(submitError?.message ?? tr('Could not save your rating. Please try again.'));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <SafeAreaView style={styles.root}>
      <ScrollView contentInsetAdjustmentBehavior="automatic" showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        <Pressable onPress={() => router.replace('/(tabs)/progress')} style={[styles.skip, isRTL && styles.skipRTL]} hitSlop={10}><Txt variant="caption">{tr('Skip for now')}</Txt></Pressable>
        ...
        <Animated.View style={[styles.celebration, { opacity: reveal, transform: [{ translateY: reveal.interpolate({ inputRange: [0, 1], outputRange: [24, 0] }) }] }]}>
          ...
          <Txt variant="monoTag" style={{ marginTop: 20 }}>{tr('SESSION COMPLETE')}</Txt>
          <Txt style={styles.heroTitle}>{tr('You showed up.')}</Txt>
          <Txt variant="body" center style={styles.heroCopy}>{tr('That session is now part of your progress—not another workout that disappears.')}</Txt>

          <View style={[styles.impactRow, isRTL && styles.rtlRow]}>
            <Impact value={locale === 'ar' ? '+٥٠' : '+50'} label={tr('XP earned')} icon="sparkles" />
            <View style={styles.impactDivider} />
            <Impact value={(booking?.duration_min ?? 60).toLocaleString(localeTag)} label={tr('minutes')} icon="time-outline" />
            <View style={styles.impactDivider} />
            <Impact value={streak.toLocaleString(localeTag)} label={tr('week streak')} icon="flame-outline" />
          </View>
        </Animated.View>

        <View style={styles.missionCard}>
          <View style={[styles.missionTop, isRTL && styles.rtlRow]}>
            <View style={styles.missionIcon}><Ionicons name={missionProgress >= 1 ? 'trophy' : 'flag'} size={19} color={colors.primary} /></View>
            <View style={{ flex: 1 }}><Txt style={styles.missionEyebrow}>{tr('WEEKLY MISSION')}</Txt><Txt variant="bodyStrong">{missionProgress >= 1 ? tr('Mission complete') : locale === 'ar' ? `${(MISSION_TARGET - sessionsThisWeek).toLocaleString(localeTag)} ${MISSION_TARGET - sessionsThisWeek === 1 ? 'جلسة تفصلك' : 'جلسات تفصلك'} عن المكافأة` : `${MISSION_TARGET - sessionsThisWeek} session${MISSION_TARGET - sessionsThisWeek === 1 ? '' : 's'} to your bonus`}</Txt></View>
            <Txt style={styles.missionCount}>{Math.min(sessionsThisWeek, MISSION_TARGET).toLocaleString(localeTag)}/{MISSION_TARGET.toLocaleString(localeTag)}</Txt>
          </View>
          <View style={styles.missionTrack}><View style={[styles.missionFill, { width: `${Math.max(5, missionProgress * 100)}%` }]} /></View>
          <Txt variant="caption" style={{ marginTop: 8 }}>{missionProgress >= 1 ? tr('150 mission XP unlocked in your progress.') : tr('Complete three sessions this week to unlock 150 bonus XP.')}</Txt>
        </View>

        <View style={[styles.ratingHead, isRTL && styles.rtlRow]}>
          <View style={{ flex: 1 }}><Txt variant="monoTag">{tr('CLOSE THE LOOP')}</Txt><Txt style={styles.sectionTitle}>{locale === 'ar' ? `كيف كانت جلستك مع ${trainerName}؟` : `How was ${trainerName}?`}</Txt></View>
          {trainer?.avatar_url && <Image source={{ uri: trainer.avatar_url }} style={styles.trainerAvatar} contentFit="cover" />}
        </View>

        <View style={styles.stars} accessibilityLabel={locale === 'ar' ? `تقييم ${rating.toLocaleString(localeTag)} من ٥` : `Rating ${rating} out of 5`}>
          {[1, 2, 3, 4, 5].map((number) => (
            <Pressable key={number} onPress={() => setRating(number)} hitSlop={6} accessibilityLabel={locale === 'ar' ? `${number.toLocaleString(localeTag)} نجوم` : `${number} stars`}>
              <Ionicons name={number <= rating ? 'star' : 'star-outline'} size={39} color={colors.primary} />
            </Pressable>
          ))}
        </View>

        <Txt variant="label">{tr('What stood out?')}</Txt>
        <View style={[styles.tags, isRTL && styles.rtlWrap]}>
          {TAGS.map((tag) => (
            <Pressable key={tag} onPress={() => toggleTag(tag)} style={[styles.tag, tags.includes(tag) && styles.tagOn]}>
              <Txt style={[styles.tagText, tags.includes(tag) && { color: colors.white }]}>{tr(tag)}</Txt>
            </Pressable>
          ))}
        </View>

        <View style={styles.commentWrap}>
          <TextInput placeholder={tr('Help the next client know what to expect…')} placeholderTextColor={colors.textDim} value={comment} onChangeText={setComment} multiline maxLength={500} style={[styles.comment, isRTL && styles.inputRTL]} />
          <Txt style={[styles.characterCount, isRTL && styles.characterCountRTL]}>{comment.length.toLocaleString(localeTag)}/{'500'.replace(/\d/g, (digit) => locale === 'ar' ? '٠١٢٣٤٥٦٧٨٩'[Number(digit)] : digit)}</Txt>
        </View>

        {photo ? (
          <View style={styles.photoWrap}>
            <Image source={{ uri: photo }} style={styles.photo} contentFit="cover" />
            <Pressable style={[styles.photoRemove, isRTL && styles.photoRemoveRTL]} onPress={() => setPhoto(null)} hitSlop={8} accessibilityLabel={tr('Remove progress photo')}><Ionicons name="close-circle" size={26} color={colors.white} /></Pressable>
          </View>
        ) : (
          <Pressable style={[styles.photoButton, isRTL && styles.rtlRow]} onPress={pickPhoto}>
            <View style={styles.photoIcon}><Ionicons name="camera-outline" size={20} color={colors.primary} /></View>
            <View style={{ flex: 1 }}><Txt variant="bodyStrong">{tr('Save a progress moment')}</Txt><Txt variant="caption" style={{ marginTop: 3 }}>{tr('Private by default and attached to your journey.')}</Txt></View>
            <Ionicons name="add" size={20} color={colors.primary} />
          </Pressable>
        )}

        {!isBackendConfigured && (
          <Card><View style={[styles.coachNoteTop, isRTL && styles.rtlRow]}><Ionicons name="chatbubble-ellipses" size={15} color={colors.primary} /><Txt variant="bodyStrong">{locale === 'ar' ? `ملاحظة من ${trainerName}` : `A note from ${trainerName}`}</Txt></View><Txt variant="body" style={{ marginTop: 8 }}>{tr('“Ready on time, focused, and strong through the final set. Great work.”')}</Txt></Card>
        )}
      </ScrollView>

      <View style={styles.footer}>
        {error && <Txt variant="caption" color={colors.danger} center style={{ marginBottom: 9 }}>{error}</Txt>}
        <Button title={tr(rating ? 'Save & see my progress' : 'Choose a rating')} icon={isRTL ? 'arrow-back' : 'arrow-forward'} onPress={submit} disabled={rating === 0 || (!isPreview && booking?.status !== 'completed')} loading={busy} />
      </View>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  characterCountRTL: { alignSelf: 'flex-start' },
  ...
  photoRemoveRTL: { right: undefined, left: 10 },
